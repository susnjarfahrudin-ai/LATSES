"""Legacy measurement-device API kept as a compatibility facade.

The canonical SCI measurement record lives in ``scientific.measurement``;
these device helpers remain here so existing analysis/GUI imports continue to
resolve while callers migrate to the canonical contract.
"""
...
class MeasurementDevice:
    ...
    def __init__(self, name: str, device_type: str, unit: Unit, accuracy_spec: AccuracySpec,
                 min_range: float, max_range: float, calibration_offset: float = 0.0,
    ...
    def measure(self, raw_value: float) -> PhysicalQuantity:
        ...
        return PhysicalQuantity(value=corrected_value, uncertainty=uncertainty,
                                unit=self.unit, sko_uuid=self.uuid)
...
def create_pitot_tube(name: str = "Standard Pitot Tube") -> MeasurementDevice:
    ...
    return MeasurementDevice(name, "Pitot Tube", meter_per_second,
                             AccuracySpec(0.015, 0.1), 1.0, 40.0)


def create_diff_pressure_sensor(name: str = "Plenum DP Sensor") -> MeasurementDevice:
    pascal = Unit("pascal", "Pa", MASS / (LENGTH * (TIME**2)))
    return MeasurementDevice(name, "Differential Pressure Transmitter", pascal,
                             AccuracySpec(0.005, 1.0), 0.0, 2000.0)
