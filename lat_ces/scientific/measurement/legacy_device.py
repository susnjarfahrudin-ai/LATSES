...
class MeasurementDevice:
    """Compatibility measurement instrument; new evidence flows use Measurement."""
...
def create_pitot_tube(name: str = "Standard Pitot Tube") -> MeasurementDevice:
    ...
    return MeasurementDevice(name, "Pitot Tube", meter_per_second, AccuracySpec(0.015, 0.1), 1.0, 40.0)


def create_diff_pressure_sensor(name: str = "Plenum DP Sensor") -> MeasurementDevice:
    pascal = Unit("pascal", "Pa", MASS / (LENGTH * (TIME**2)))
    return MeasurementDevice(name, "Differential Pressure Transmitter", pascal, AccuracySpec(0.005, 1.0), 0.0, 2000.0)
