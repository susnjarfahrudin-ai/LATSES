...
class CyberFortress:
    ...
    The fortress does not reimplement any security primitive. Callers inject
    the IPC channel and may inject policy engines for deterministic testing.
    The intended request order is admission control first, then authenticated
    IPC processing, with threat scoring recording rejected boundary events.
    """

    def __init__(
        self,
        ipc_channel: SignedIPCChannel,
        *,
        rate_limiter: TokenBucketRateLimiter | None = None,
        threat_engine: ThreatScoreEngine | None = None,
    ) -> None:
    ...
    def receive(self, address: str, packet: bytes, *, cost: float = 1.0, now: float | None = None) -> dict[str, Any]:
        """Pass a request through the unified admission and authenticated IPC boundary."""
