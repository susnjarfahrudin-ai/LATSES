"""Room-selection and underfloor-zone behavior for the MEP workspace.

This module is deliberately an adapter around EngineeringMEPWorkspaceApp so the
main MEP UI layout remains unchanged while the canonical BuildingModel.mep
registry gains synchronized room selection and spatial underfloor zones.
"""
...
def _room_rows(app):
    if app.workflow is None:
        return []
    level = next((x for x in app.workflow.model.levels.values() if x.level_id == app.active_level_id), None)
    if level is None:
        return []
    return [(room.room_id, f"{room.room_id} · {room.name}") for room in level.rooms.values()]


def _find_room_combo(app):
    for combo_id, (values, display, variable) in getattr(app, "_combo_maps", {}).items():
        if variable is getattr(app, "heating_room_var", None):
            return app._combo_widgets.get(combo_id)
    return None


def install_room_zone_behavior(cls):
    """Patch the existing workspace with synchronized room selection and zones."""
    ...
    def _init(self, *args, **kwargs):
        ...
        self.zone_mode_var = ttk.StringVar(value=_ZONE_LABELS["full"])
        self._zone_display = tuple(_ZONE_LABELS[m] for m in ZONE_MODES)
        heat_tab = self.tabs.nametowidget(self.tabs.tabs()[0])
        zone_box = ttk.LabelFrame(heat_tab, text="Podna zona", padding=8)
        zone_box.pack(fill="x", pady=(8, 0))
        self.zone_mode_combo = ttk.Combobox(zone_box, textvariable=self.zone_mode_var, state="readonly", values=self._zone_display, width=24)
        self.zone_mode_combo.pack(side="left")
        ttk.Label(zone_box, text="Za veće prostorije: cijela ili 1/2 A/B; oba pola mogu se definirati kao dvije zone.", foreground="#475569", wraplength=620).pack(side="left", padx=10)

    def _sync_room_controls(self):
        rows = _room_rows(self)
        room_ids = [r[0] for r in rows]
        displays = [r[1] for r in rows]
        if self.workflow is None:
            return
        maps = getattr(self, "_combo_maps", {})
        widgets = getattr(self, "_combo_widgets", {})
        for combo_id, (values, display, variable) in list(maps.items()):
            if variable is getattr(self, "heating_room_var", None):
                combo = widgets.get(combo_id)
                if combo is not None:
                    maps[combo_id] = (tuple(room_ids), tuple(displays), variable)
                    combo["values"] = tuple(displays)
                    if self.selected_room_id in room_ids:
                        idx = room_ids.index(self.selected_room_id)
                        combo.current(idx)
                        variable.set(room_ids[idx])
                    elif room_ids:
                        combo.current(0)
                        variable.set(room_ids[0])

    def _refresh_context(self):
        original_refresh_context(self)
        _sync_room_controls(self)
    ...
    def _combo_selected(self, combo, labels):
        ...
        variable = self._combo_maps[id(combo)][2]
        if variable is getattr(self, "heating_room_var", None):
            self.selected_room_id = variable.get()
            self.room_var.set(variable.get())
    ...
    def _save_underfloor(self):
        ...
        if not systems:
            return
        system = systems[-1]
        label = self.zone_mode_var.get()
        mode = next((key for key, value in _ZONE_LABELS.items() if value == label), "full")
    ...
    def _draw_context(self):
        ...
        for system in registry.all_underfloor_systems:
            ...
            for zone in room_zones:
                ...
                x1, y1 = xy(zx1, zy1)
                x2, y2 = xy(zx2, zy2)
                self.canvas.create_rectangle(x1 + 4, y2 + 4, x2 - 4, y1 - 4, outline="#1d4ed8", width=2, dash=(5, 3), tags="ufh-zone")
                spacing_px = max(6.0, system.pipe_spacing_m * scale)
                y = y2 + 8
                while y < y1 - 8:
                    self.canvas.create_line(x1 + 8, y, x2 - 8, y, fill="#2563eb", width=1, tags="ufh-schematic")
                    y += spacing_px
