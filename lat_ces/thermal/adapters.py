"""Optional adapters for external project workflow systems.

These adapters consume WorkflowAction only. They are deliberately outside the
scientific/input-validation core and require no credentials in source code.
"""
...
from .workflow import WorkflowAction, WorkflowAdapter


class EmailWorkflowAdapter(WorkflowAdapter):
    """SMTP adapter for INPUT_BLOCKER notifications.

    SMTP host, sender and credentials are supplied through environment
    variables. Notification failure is raised to the application layer and
    must not mutate the validation result.
    """
    ...
    def dispatch(self, action: WorkflowAction) -> None:
        ...
        recipient = os.getenv(f"LATCES_ROLE_EMAIL_{action.target.upper().replace(' ', '_').replace('/', '_')}")
        if not recipient:
...
class DeepLinkAdapter:
    """Builds stable application URLs for a blocked input."""
