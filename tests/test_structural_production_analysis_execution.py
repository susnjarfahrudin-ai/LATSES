...
def _request() -> ProductionStructuralAnalysisRequest:
    ...
    return ProductionStructuralAnalysisRequest(
        model=model,
...
def test_production_model_executes_existing_solver_and_preserves_identity() -> None:
    ...
    assert binding.result.result_id == request.result_id
...
def test_production_analysis_request_is_immutable() -> None:
    request = _request()
    with pytest.raises(FrozenInstanceError):
        request.result_id = "changed"


def test_production_analysis_rejects_wrong_request_type() -> None:
    with pytest.raises(TypeError, match="ProductionStructuralAnalysisRequest"):
        run_production_structural_analysis(object())
