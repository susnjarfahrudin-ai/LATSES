...
from examples.reference_house_validation import build_reference_house
...
def test_reference_house_is_physically_valid():
    ...
    assert {room.name for room in house.rooms} == {
        "Hodnik", "Kuhinja", "Dnevni boravak", "Soba 1", "Soba 2"
    }
...
def test_reference_house_preserves_product_and_material_identity():
    ...
    assert thermo.dimensions_m == (0.25, 0.25, 0.30)
    assert thermo.thermal_conductivity_w_mk == 0.18
