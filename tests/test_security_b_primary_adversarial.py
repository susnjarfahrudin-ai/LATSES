...
def _resign(packet: bytes, mutate) -> bytes:
    ...
    canonical = json.dumps(decoded["envelope"], sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
...
def test_b_primary_survives_five_minute_four_dimension_adversarial_burst() -> None:
    ...
    while time.monotonic() - start < duration:
        ...
        if 0.12 < t < 0.20: wave = 1.20
        elif 0.20 <= t < 0.28: wave = 0.80
        elif 0.28 <= t < 0.45: wave = 1.12 if phase % 3 else 0.88
        elif 0.45 <= t < 0.65: wave = 1.18 if phase % 4 in (0, 1) else 0.82
        elif 0.65 <= t < 0.82: wave = 1.195 if phase % 2 == 0 else 0.805
        elif 0.82 <= t: wave = 1.02 if phase % 2 == 0 else 0.98
        values = {"frequency": 100.0 * wave, "volume": 100.0 * (1.0 + ((wave - 1.0) * 0.75)), "concurrency": 100.0 * (1.0 + ((wave - 1.0) * 1.10)), "novelty": 100.0 * (1.0 - ((wave - 1.0) * 0.90))}
        ...
        try: b.receive("203.0.113.42", attack_packets[phase % len(attack_packets)], now=time.time())
    ...
    assert not a.adaptive_defense.records()
    assert guard_b.baseline == guard_a.baseline == {"frequency": 100.0, "volume": 100.0, "concurrency": 100.0, "novelty": 100.0}
