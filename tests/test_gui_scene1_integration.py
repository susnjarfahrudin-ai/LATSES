...
from lat_ces.gui_architecture.adapters import Scene1Adapter
from lat_ces.gui_scene1 import Scene1CompleteBuildingWorkspaceApp
from lat_ces.scene1_gui_mixin import Scene1GUIMixin


def test_scene1_gui_uses_stable_scene_adapter_boundary():
    assert Scene1CompleteBuildingWorkspaceApp.__mro__[1].__name__ == "CompleteBuildingWorkspaceApp"
    assert issubclass(Scene1CompleteBuildingWorkspaceApp, Scene1GUIMixin)
    assert "_scene1_adapter" in Scene1GUIMixin._init_scene1_gui.__code__.co_names
    assert "present_model" in Scene1GUIMixin.show_scene1.__code__.co_names
    assert Scene1Adapter.status.value == "CONNECTED"
...
def _make_model() -> BuildingModel:
    ...
    for name, start, end in (
        ...
        floor_plan.add_wall(Wall(name, Segment2D(start, end), thickness=0.20, exterior=True, load_bearing=True))

    level = Level("Prizemlje", 0.0, 2.8, length_m=10.0, width_m=8.0, floor_plan=floor_plan)
...
def test_scene1_show_uses_canonical_snapshot_and_does_not_write_model():
    ...
    before = (model.name, tuple(model.levels), level.length_m, level.width_m, tuple(level.floor_plan.walls))
    ...
    app._scene1_adapter = Scene1Adapter()
    ...
    assert app.status_var.value == "Scene 1: BuildingModel → Scene1Adapter → 2D presentation"
    assert (model.name, tuple(model.levels), level.length_m, level.width_m, tuple(level.floor_plan.walls)) == before
